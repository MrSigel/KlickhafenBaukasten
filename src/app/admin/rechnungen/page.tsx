...
export default async function InvoicesPage({ searchParams }: { searchParams: Promise<{ success?: string }> }) {
        ...
        {(data || []).map((row) => (
          <div key={row.id} className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm hover:border-cyan-700">
            ...
            <div className="mt-4 flex flex-col gap-3 sm:flex-row">
              <a href={`/api/admin/invoices/${row.id}/pdf`} className="inline-flex min-h-10 items-center justify-center rounded-md border border-slate-300 bg-white px-4 py-2 text-sm font-semibold text-slate-950 hover:border-cyan-700 hover:text-cyan-800">
