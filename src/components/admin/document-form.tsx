  ...
  return (
    <form action={action} className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <Field label="Status"><select name="status" className="input">{(type === "offer" ? ["draft", "sent", "accepted", "rejected", "expired"] : ["sent", "paid", "overdue", "cancelled"]).map((s) => <option key={s}>{s}</option>)}</select></Field>
